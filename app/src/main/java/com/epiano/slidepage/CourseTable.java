...
import com.epiano.bean.Lesson;
import com.epiano.bean.LessonDAO;
import com.epiano.commutil.R;
import com.epiano.utils.ColorUtils;
import com.epiano.utils.DateUtil;
import com.epiano.utils.DensityUtils;
import com.epiano.utils.GestureListener;
import com.epiano.utils.ScreenUtils;
import com.epiano.view.ScrollViewExtend;
...
//			Thread thread = new Thread(new TableHandler());
//			thread.start();
		}
	...
	class TableHandler implements Runnable {
		@Override
		public void run() {
			try{
				lessons = jsonUtils.parseLessonFromJson(getSongList("http://121.42.153.237/liudian/lession.php"));
...
//	 * @param weekday 周几
	...
	public void addCourseView(Lesson bean){
		...
		int[] sParams = DateUtil.getTime(bean.getEstablishTime().toString());
		int[] eParams = DateUtil.getTime(bean.getLastActTime().toString());
...
//			bean.status == ClientCode.YS?mContext.getResources().getColor(background[0]):mContext.getResources().getColor(background[DateUtil.getWeekDay()])
			...
			StringBuilder builder = new StringBuilder();
			builder.append(bean.getEstablishTime());
			builder.append("\n");
			builder.append(" ");
			builder.append(bean.getUserName());
...
//		builder.add("enum", endnum+"");
		...
		try {
			...
			System.out.println("ffffffff"+data);
