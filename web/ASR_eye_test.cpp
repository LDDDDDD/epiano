...
main()
{
	...
	strcpy(ASRRecCfg.DBName, ASR_DB_NAME);
	strcpy(ASRRecCfg.HtmlSubDir, "");
...
{
	static int count = 0;
	printf("\nhhhhhhhhhhhhhhhhhhhhhh ASREventGenHtml(), called:%d.", ++count);
...
int ASRAnalyser::InitDB()
{
	...
	mysql_init(&mysql);
	mysql_real_connect(&mysql, "localhost", "root", "huawei123", "test", 3306, NULL, 0);
...
int ASREventRecordStatRep_Internel(char *DBName, ASR_EVENT * pEvent)
{
	...
	if (pASRAnalyser->mASR_REC_CFG.DBName[0] == 0)
	{
		...
		ASR_REC_CFG ASRRecCfg;
		strcpy(ASRRecCfg.DBName, "ASR_DB");
