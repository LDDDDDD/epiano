...
//↓读取指定记录数
$result=mysql_query("select * from SongTbl order by idx limit $offset,$pagesize",$conn);//取得—当前页—记录集！
...
//echo "转到第 <INPUT maxLength=3 size=3 value=".($page+1)." name=gotox> 页 <INPUT hideFocus onclick=\"location.href='?page=$('#gotox').val()';\" type=button value=Go name=cmd_goto>"; 
echo "</p>";
...
while($row = mysql_fetch_array($result))
{
  ...
  $songname  = iconv("GB2312","UTF-8//IGNORE",$row['songname']);
  $writer  = iconv("GB2312","UTF-8//IGNORE",$row['writer']);
  
  echo "<tr>";
  //echo "<td>" . $row['ASRDatetime'] . "</td>";
	echo "<td width=\"100px\" align=\"center\">" . $row['idx'] . "</td>";  
  //echo "<td>" . $row['songname'] . "</td>";
  if (!empty($songname))
  {
  	$filename = "./sdir/" . $row['idx'] . ".sco";
  	if(file_exists($filename))
  	{
  		echo "<td width=\"500px\" align=\"center\"><a href=\"sdir\\{$row['idx']}.sco\">" . $songname . "</a></td>";  
  	}
  	else
  	{
  		echo "<td width=\"500px\" align=\"center\">" . $songname . "</a></td>";  
  	}
  }
  else
  {
  	echo "<td align=\"center\">" . "空" . "</td>";  
  }	
  echo "<td width=\"300px\" align=\"center\">" . $writer . "</td>";  
/*
